...
DEFAULT_TEST_COMMAND = "coverage run -m pytest && coverage json -o target/coverage/coverage.json"


def parser() -> argparse.ArgumentParser:
    value = argparse.ArgumentParser(description="Calculate CRAP scores for Python functions.")
    value.add_argument("filters", nargs="*", help="Only analyze source paths that contain one of these fragments.")
    value.add_argument("--root", type=Path, default=Path("."), help="Project root. Default: current directory.")
    value.add_argument("--coverage", type=Path, default=DEFAULT_COVERAGE, help="coverage.py JSON report.")
    value.add_argument("--test-command", default=DEFAULT_TEST_COMMAND, help="Command that creates the coverage report.")
    value.add_argument("--no-test", action="store_true", help="Do not run tests. Read the existing coverage report.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    except (OSError, SyntaxError, ValueError, RuntimeError, json.JSONDecodeError) as error:
        print(f"crap4python: {error}", file=sys.stderr)
        return 1
